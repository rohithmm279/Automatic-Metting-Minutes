"""Gemini Benchmark -- SC001 through SC070.

Evaluates Gemini as the primary AI candidate for S5 using the exact same:
  - SC001-SC070 transcripts and ground truth
  - TranscriptProcessor.clean_transcript()
  - Production baseline 10-rule prompt (build_meeting_analysis_prompt)
  - Deterministic Pydantic validation (MeetingValidator, MeetingOutput)
  - Lexical token-Jaccard (threshold >= 0.5)
  - Semantic similarity (SemanticMatcher, all-MiniLM-L6-v2, threshold >= 0.5)
  - Hallucination checks (owners, deadlines)
  - Macro and Micro PRF metrics

Features:
  - Configurable model via GEMINI_MODEL env var or --model flag (default: gemini-3.5-flash)
  - Secure API key loading from GEMINI_API_KEY / backend/.env (never hardcoded)
  - Rate-limit and transient error handling with exponential backoff
  - Incremental checkpointing (gemini_eval_SC001_SC070_checkpoint.json)
  - Support for --smoke flag (runs only SC001 for verification)
"""
...
CHECKPOINT_FILE = ROOT / "gemini_eval_SC001_SC070_checkpoint.json"
RAW_OUT_FILE = ROOT / "gemini_eval_SC001_SC070_raw.json"
METRICS_OUT_FILE = ROOT / "gemini_eval_SC001_SC070_metrics.json"

for _p in (ROOT, BACKEND):
    if str(_p) not in sys.path:
        sys.path.insert(0, str(_p))

# Load environment from backend/.env if not already in environment
...
if not api_key:
    raise ValueError("GEMINI_API_KEY environment variable is not set. Please set it or add to backend/.env.")
...
def call_gemini(client: Any, model_name: str, prompt: str, max_retries: int = 5) -> tuple[str, float, dict]:
    """Call Gemini generate_content with retries on transient errors."""
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            if "429" in err_msg or "RESOURCE_EXHAUSTED" in err_msg or "503" in err_msg or "UNAVAILABLE" in err_msg:
                wait_sec = 2 ** attempt
                print(f"    [Retry {attempt}/{max_retries}] Transient API status: {err_msg[:60]}... Waiting {wait_sec}s")
                time.sleep(wait_sec)
            else:
                raise exc
    raise last_err or RuntimeError("Max retries exceeded calling Gemini API")
...
# ── Benchmark Runner ───────────────────────────────────────────────────────────
def run_gemini_benchmark(model_name: str, smoke_only: bool = False):
    print("=" * 75)
    print(f"  GEMINI BASELINE BENCHMARK ({model_name})")
    print(f"  Scope: {'SMOKE TEST (SC001 ONLY)' if smoke_only else 'SC001 -> SC070'}")
    print("=" * 75)

    client = create_gemini_client()
    target_meetings = ["SC001"] if smoke_only else ALL_MEETINGS

    cached_results = {} if smoke_only else load_checkpoint()
    if cached_results and not smoke_only:
        print(f"Loaded {len(cached_results)} previously completed evaluations from checkpoint.")

    results: list[dict] = []
    total_meetings = len(target_meetings)

    for idx, meeting_id in enumerate(target_meetings, 1):
        if not smoke_only and meeting_id in cached_results and cached_results[meeting_id].get("parse_ok") is not None:
        ...
        try:
            raw_output, elapsed, token_usage = call_gemini(client, model_name, prompt)
        except Exception as exc:
            print(f"[{idx:02d}/{total_meetings}] {meeting_id} ❌ Gemini API call failed: {exc}")
            rec = {
                "meeting_id": meeting_id,
                "model": model_name,
                "parse_ok": False,
    ...
    if smoke_only:
        print("\n" + "=" * 75)
        print("  SMOKE TEST RESULT (SC001)")
        print("=" * 75)
        r = results[0]
        print(f"  Meeting ID          : {r['meeting_id']}")
        print(f"  Model               : {r['model']}")
        print(f"  Latency             : {r['elapsed_s']}s")
        print(f"  JSON Parse OK       : {r['parse_ok']}")
        print(f"  Pydantic Valid      : {r['pydantic_ok']}")
        print(f"  Summary Present     : {r['has_summary']}")
        print(f"  Action Items F1     : Lex={r['lex_ai_f1']:.4f}, Sem={r['sem_ai_f1']:.4f}")
        print(f"  Decisions F1        : Lex={r['lex_dec_f1']:.4f}, Sem={r['sem_dec_f1']:.4f}")
        print(f"  Unresolved Issues F1: Lex={r['lex_iss_f1']:.4f}, Sem={r['sem_iss_f1']:.4f}")
        print(f"  Hallucinations      : {r['hallucination_count']}")
        print("=" * 75)
        return results

    # ── Full Aggregate Computation ────────────────────────────────────────────
    ...
    print("\n" + "=" * 75)
    print(f"  GEMINI ({model_name}) FULL BENCHMARK RESULTS: SC001 -> SC070")
    print("=" * 75)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Gemini SC001-SC070 benchmark")
    parser.add_argument("--model", type=str, default=os.getenv("GEMINI_MODEL", "gemini-3.5-flash"), help="Gemini model name")
    parser.add_argument("--smoke", action="store_true", help="Run only SC001 smoke test")
    args = parser.parse_args()

    run_gemini_benchmark(model_name=args.model, smoke_only=args.smoke)
